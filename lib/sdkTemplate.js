 ...
 * @LastEditTime: 2020-12-08 11:12:55
 */
...
const { defaultGitGroupObj, gitUrl, isCreateGitRes } = require("./config");
...
async function getSDKtemplate() {
  ...
  return { description, keywords, gitAuthor };
}
/*
 * @name: getInput
 ...
 * @return {string}
 */
async function createGitProject_SDK({
  name = "",
  sdkParams,
  namespace_id = defaultGitGroupObj.id,
} = {}) {
  const spinner = ora(" Creating git repository...");
  spinner.start();
  const description = sdkParams.description;
  return api
    .post(`${gitUrl}/api/v4/projects`, {
      name,
      namespace_id,
      description,
    })
    ...
    .catch((err) => {
      spinner.fail(`Failed to create gitlab warehouse!`);
