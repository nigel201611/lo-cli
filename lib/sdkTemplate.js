 ...
 * @LastEditTime: 2020-12-10 10:10:24
 */
...
async function createGitProject_SDK({ name = "", sdkParams } = {}) {
  ...
  //   });
  // github api
  const octokit = new Octokit({ auth: privateToken });
  try {
    const response = await octokit.request(`POST /orgs/${orgsName}/repos`, {
      org: orgsName,
      name,
    });
  } catch (err) {
    console.log("octokit err", err);
    return null;
  }
}
...
async function initGitRes(name, sdkParams, options) {
  ...
  console.log(111, GIT_SKD_PROJECT_NAME);
